...
*/
	 ...
	 * @throws UnauthorizedException
	 */
	@ApiMethod(name = "joinFamily")
	public FamilyMember joinFamily(Family pfamily) throws UnauthorizedException {
		FamilyMember familymember=null;
	    ...
	    Family family=Family.getFamily(pfamily.getName());
		
		if (family != null) {
			if (!family.getToken().equals(pfamily.getToken()));
				throw new UnauthorizedException("Token mismatch on existing Family");
		} else {
			family=Family.makeFamily(user, pfamily.getName());
		}

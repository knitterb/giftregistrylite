...
public class GiftEndpoint {
	...
	public CollectionResponse<Gift> listGift(
		...
		try {
			...
			execute = (List<Gift>) query.getResultList();
	...
	public void removeGift(@Named("id") Long id) {
		EntityManager mgr = getEntityManager();
		try {
			Gift gift = mgr.find(Gift.class, id);
			mgr.remove(gift);
		} finally {
			mgr.close();
		}
	...
	private boolean containsGift(Gift gift) {
		...
		try {
			Gift item = mgr.find(Gift.class, gift.getId());
			if (item == null) {
				contains = false;
			}
		} finally {
